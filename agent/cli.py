...
Explicit commands
    ...
    coding-agent springboot SERVICE_NAME [--port 8080] [--db h2|postgres|mysql|none]
                            [--package com.example] [--gradle|--maven]
                            [--swagger/--no-swagger] [--docker/--no-docker]
                            [--dir DIR]
    coding-agent login
    coding-agent logout
    coding-agent whoami
...
# ── Subcommand: springboot ────────────────────────────────────────────────────

@app.command(name="springboot")
def springboot(
    service_name: str = typer.Argument(..., help="Microservice name, e.g. payment-service"),
    port: int = typer.Option(8080, "--port", "-p", help="HTTP port"),
    package: str = typer.Option("", "--package", help="Base Java package (auto-derived if empty)"),
    build_tool: str = typer.Option("gradle", "--gradle", "--maven",
                                   help="Build tool: 'gradle' or 'maven'",
                                   envvar="CODING_AGENT_BUILD_TOOL"),
    db: str = typer.Option("h2", "--db",
                           help="Database: none | h2 | postgres | mysql"),
    add_swagger: bool = typer.Option(True,  "--swagger/--no-swagger",
                                     help="Include Swagger/OpenAPI (springdoc)"),
    add_docker: bool  = typer.Option(True,  "--docker/--no-docker",
                                     help="Include Dockerfile + docker-compose.yml"),
    directory: Path   = typer.Option(Path.cwd(), "--dir", "-d", show_default=False,
                                     help="Parent directory for the new project"),
    do_git: bool      = typer.Option(True, "--git/--no-git",
                                     help="Initialise a git repo and commit the scaffold"),
) -> None:
    """
    Scaffold a production-ready Spring Boot microservice.

    Creates the full project structure with build config, controllers,
    exception handling, DTOs, optional Swagger/OpenAPI, Docker support,
    and a Makefile — ready to run with `make run`.

    \\b
    Examples:
      coding-agent springboot payment-service --port 8081 --db h2
      coding-agent springboot order-service   --port 8082 --db postgres --docker
      coding-agent springboot auth-service    --port 8083 --maven --no-swagger
    """
    from .springboot_scaffolder import SpringBootConfig, scaffold_spring_boot
    from .springboot_scaffolder import print_scaffold_plan, print_scaffold_summary

    # ── Normalise build tool (typer flag hack: if --maven passed, build_tool="maven") ──
    # typer doesn't natively support mutually exclusive flags → we parse sys.argv
    if "--maven" in sys.argv:
        build_tool = "maven"
    elif "--gradle" in sys.argv:
        build_tool = "gradle"

    # ── Validate db ───────────────────────────────────────────────────────────
    valid_dbs = {"none", "h2", "postgres", "mysql"}
    if db not in valid_dbs:
        err_console.print(f"✗ --db must be one of: {', '.join(sorted(valid_dbs))}")
        raise typer.Exit(code=1)
    ...
    print_scaffold_plan(cfg)

    if not Confirm.ask("Generate scaffold?", default=True):
        console.print("[dim]Aborted.[/]")
        raise typer.Exit(code=0)

    console.print(f"\n[bold cyan]Scaffolding[/] [green]{service_name}[/] …\n")
    paths = scaffold_spring_boot(cfg)

    # ── Git init + commit ──────────────────────────────────────────────────────
    if do_git:
        try:
            import git as _git
            repo_path = cfg.project_root
            if not (repo_path / ".git").exists():
                git_ops.init_repo(repo_path)
            repo = _git.Repo(str(repo_path))
            repo.git.add(A=True)
            repo.index.commit(
                f"feat: scaffold {service_name} — Spring Boot {service_name}\n\n"
                f"Generated by coding-agent springboot"
            )
            console.print("\n[green]✓[/] Committed scaffold to git.")
        except Exception as exc:  # noqa: BLE001
            console.print(f"[yellow]⚠ Git commit skipped: {exc}[/]")

    print_scaffold_summary(cfg, len(paths))
